...
//                LoggerService.getInstance().log("send HTTP request: to " + test.getPseudo() + " and port " + SessionService.getInstance().getHttp_port() + " with users: " + test);
                m_request.exceptionally((e) -> {
                            LoggerService.getInstance().log("Removing user from connectedUsers list: " + test.getPseudo());
